...
class VisorImagenes:
    def __init__(self, ventana):
        ...
        self.contrasteS = Scale(ventana, from_=-1, to=1, resolution=0.1, orient=tk.HORIZONTAL, command=self.contraste)
        self.contrasteS.place(x=1180 ,y= 220)

        self.zo=tk.BooleanVar()
        self.zc=tk.BooleanVar()

        checkZonaOscura = tk.Checkbutton(ventana, text="Zonas oscuras", variable=self.zo)
        checkZonaOscura.place(x=1300, y=220)

        checkZonasClaras= tk.Checkbutton(ventana, text="Zonas claras", variable=self.zc)
    ...
    def brillo(self, valor):
        if not self.img_original:
            messagebox.showwarning("Cuidado", "Primero selecciona una imagen")
            return
        imgBrillo = self.img_original.copy()
        bri = float(valor)
        imgBrillo = aux.ajusteBrillo(imgBrillo, bri)
    ...
    def contraste(self, valor):
        if not self.img_original:
            messagebox.showwarning("Cuidado", "Primero selecciona una imagen")
            return
        imgCon = self.img_original.copy()
        cont = float(valor)
        if self.zo.get() and self.zc.get():
            messagebox.showwarning("Cuidado", "No se puede aplicar ambos ajustes a la vez")
            return
        if self.zo.get() == True:
            imgCon = aux.ajusteContraste(imgCon, cont, 1)
        elif self.zc.get() == True:
            imgCon = aux.ajusteContraste(imgCon, cont, 2)

        self.img_tk = ImageTk.PhotoImage(imgCon)
        self.label_img.config(image=self.img_tk, text="")
        self.label_img.image = self.img_tk
