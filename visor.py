...
class VisorImagenes:
    def __init__(self, ventana):
        ...
        boton_explorar = tk.Button(ventana, text="Explorar", command=self.explorar_principal)
        ...
        checkZonaOscura = tk.Checkbutton(ventana, text="Zonas oscuras", activebackground ="MediumPurple1", variable=self.zo, 
                                        command=self.aplicar_ajustes)
        checkZonaOscura.place(x=1300, y=220)

        checkZonasClaras= tk.Checkbutton(ventana, text="Zonas claras", activebackground ="MediumPurple1", variable=self.zc, 
                                        command=self.aplicar_ajustes)
        checkZonasClaras.place(x=1300, y=245)

        #binarizacion con slider
        label_bin = tk.Label(ventana, text="Umbral \nbinarizacion:")
        ...
        RGBlabel = tk.Label(ventana, text= "RGB:",fg = "DarkOrchid4", font = ("Helvetica", 10, "bold"))
        RGBlabel.place(x=1100, y=355)

        checkRojo = tk.Checkbutton(ventana, text = "Capa Roja", variable = self.rojo, command=self.aplicar_ajustes)
        checkRojo.place(x = 1100, y = 375)

        checkVerde = tk.Checkbutton(ventana, text = "Capa Verde", variable = self.verde, command=self.aplicar_ajustes)
        checkVerde.place(x = 1100, y = 400)

        checkAzul = tk.Checkbutton(ventana, text = "Capa Azul", variable = self.azul, command=self.aplicar_ajustes)
        ...
        CMYlabel = tk.Label(ventana, text= "CMY:",fg = "DarkOrchid4", font = ("Helvetica", 10, "bold"))
        CMYlabel.place(x=1300, y=355)

        checkCian = tk.Checkbutton(ventana, text = "Capa Cian", variable = self.cian, command=self.aplicar_ajustes)
        checkCian.place(x = 1300, y = 375)

        checkMagenta = tk.Checkbutton(ventana, text = "Capa Magenta", variable = self.magenta, command=self.aplicar_ajustes)
        checkMagenta.place(x = 1300, y = 400)

        checkAmarillo = tk.Checkbutton(ventana, text = "Capa Amarillo", variable = self.amarillo, command=self.aplicar_ajustes)
        checkAmarillo.place(x = 1300, y = 425)

        #negativo (me gustaria moverlo)
        self.neg = tk.BooleanVar()
        checkNegativo = tk.Checkbutton(ventana, text = "Negativo", variable=self.neg, command=self.aplicar_ajustes)
        checkNegativo.place(x = 1100, y = 325)

        #fusionar
        fusionlabel = tk.Label(ventana, text= "Fusionar: ", fg = "DarkOrchid4", font = ("Helvetica", 10, "bold"))
        fusionlabel.place(x=1100, y=470)

        boton_cargarSegunda = tk.Button(ventana, text="1. Cargar segunda imagen", command=self.explorar_fusion)
        boton_cargarSegunda.place(x = 1100, y = 500)

        self.ruta_fusion = None
        self.rutaFusionlabel = tk.Label(ventana, text="Ruta 2:", bg = "MediumPurple1")
        self.rutaFusionlabel.place(x = 1300, y = 500, width = 250, height=20)

        label_fus = tk.Label(ventana, text="2. Ajustar transparencia")
        ...
        #histograma
        boton_histograma = tk.Button(ventana, text="Histograma", command=self.histo)
        boton_histograma.place(x = 1100, y = 620)

        #restaurar
        boton_restaurar = tk.Button(ventana, text="Restaurar",bg = "MediumPurple1",font = ("Helvetica", 10, "bold"), command=self.reset)
